...
CIDRS_SHOULD_REJECT.forEach((cidrRange) => {
  const newIps = cidr.list(cidrRange);
  data.values = data
    .values
    .concat(newIps.map(ip => [ip, 1]));
});
// TODO: unresolved promises rejection add catch
s3.putObject({ Bucket: 'ipchecker-loaderio-payload', Key: 'payload_data', Body: JSON.stringify(data) }).promise();
