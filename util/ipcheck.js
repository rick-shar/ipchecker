...
const { checkRanges } = require('./cidr');
...
function processUpdates(jsonArray) {
  ...
  Array(4)
    .fill(null)
    .forEach((x, i) => {
      PREFIX_TABLES[i] = JSON.parse(jsonArray[i].Body);
    });
}
...
function update() {
  ...
  const promises = FILENAME_KEY_ARRAY
    .map(key => s3.getObject({ Bucket: process.env.BUCKET, Key: key }).promise());
  return Promise
    .all(promises)
    .then(processUpdates);
}
...
function checkIP({ address, level }) {
  ...
  const table = PREFIX_TABLES[level - 1];
